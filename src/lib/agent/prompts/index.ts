...
**Mobile target:** the runtime works on web AND mobile (touch input is auto-forwarded as both InputEventScreenTouch and synthesized mouse events). When the user says "mobile game" or "phone game": (1) use create_project_config with portrait viewport (e.g. 720×1280) and stretch_aspect:'keep_width'; (2) avoid hover-only interactions; (3) size touch targets ≥ 48×48 px; (4) prefer Control nodes with anchor presets over hardcoded positions. PWA install + service worker are wired so games can be added to the home screen — no separate Android/iOS build pipeline yet (web/PWA only).`;
