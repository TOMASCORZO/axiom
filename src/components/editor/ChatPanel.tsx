...
import { Send, Bot, User, Wrench, Loader2, Sparkles, AlertCircle, Box, Square, Coins, ChevronDown } from 'lucide-react';
...
export default function ChatPanel({ projectId }: ChatPanelProps) {
    ...
    const { messages, isAgentBusy, addMessage, setAgentBusy, refreshProjectFiles } = useEditorStore();
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            // Build content with meta info
            let content = data.response?.content ?? 'No response';
            if (data.meta) {
                const meta = data.meta;
                const modeBadge = data.game_mode === '3d' ? '🧊 3D' : '🎮 2D';
                content += `\n\n_${modeBadge} · ${meta.toolsExecuted} tools · ${meta.creditsUsed} créditos · ${meta.iterations} iteraciones_`;
            }
            ...
            {showModeWarning && (
                <div className="px-3 py-2 bg-amber-500/10 border-b border-amber-500/20 flex items-center gap-2 animate-fade-in">
                    ...
                    <p className="text-xs text-amber-300">
                        <span className="font-semibold">Modo 3D activado.</span> Las operaciones 3D consumen <span className="font-bold text-amber-200">3x más créditos</span> debido a la generación de modelos y escenas más complejas.
                    </p>
                </div>
            )}
                ...
                {messages.length === 0 && (
                    <div className="flex flex-col items-center justify-center h-full text-center px-4">
                        ...
                        <p className="text-xs text-zinc-600 leading-relaxed max-w-[260px]">
                            {gameMode === '3d'
                                ? 'Decime qué juego 3D querés hacer: FPS, racing, aventura... Incluiré cámara, luces y modelos 3D.'
                                : 'Decime qué juego 2D querés hacer: plataformas, RPG, puzzle... Crearé sprites, escenas y scripts.'}
                        </p>
                        {gameMode === '3d' && (
                            <div className="mt-3 px-2.5 py-1.5 bg-amber-500/10 border border-amber-500/20 rounded-lg flex items-center gap-1.5">
                                <Coins size={10} className="text-amber-400" />
                                <span className="text-[10px] text-amber-400 font-medium">3x créditos vs 2D</span>
                            </div>
                        )}
                    </div>
                )}
            ...
            <div className="p-3 border-t border-white/5">
                ...
                <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className={`text-[10px] font-medium ${gameMode === '3d' ? 'text-amber-500' : 'text-violet-500'}`}>
                        {gameMode === '3d' ? '3D Mode · 3x tokens' : '2D Mode · 1x tokens'}
                    </span>
                </div>
